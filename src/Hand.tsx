...
const variablesWithDepthsAvaliable = variablesAvaliable.flatMap(v => depthsAvaliable.map(d => `${v}⬇️${d}`));
...
export function Hand({ avaliableTabs, activeTab, setActiveTab }) {
  ...
  const [selectedVariableWithDepth, setSelectedVariableWithDepth] =
    useState<Array<string>>([variablesWithDepthsAvaliable[0]]);
  ...
  // useEffect(() => {
  //   const [selectedVariable, selectedDepth] = selectedVariableWithDepth.split('⬇️');
            ...
            <div className={classes.main}>
              ...
              <ScrollArea m="md" type="always" h="calc(100vh - 140px)">
                ...
                <MultiSelect
                  ...
                  onChange={setSelectedVariableWithDepth}
                  label="Показатель"
                />

                <Stack my="md">
                  <Text fz="sm">gfdfg;</Text>
                  <RangeSlider defaultValue={[40, 60]} color="green" />
                  <RangeSlider defaultValue={[20, 80]} color="yellow" />
                </Stack>
